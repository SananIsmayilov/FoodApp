...
class DetailFoodAdapter(val list: List<FoodModel>) : RecyclerView.Adapter<DetailFoodAdapter.FoodHolder>() {

    inner class FoodHolder(val binding: FooddetailrawBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FoodHolder {
        val view  = DataBindingUtil.inflate<FooddetailrawBinding>(LayoutInflater.from(parent.context),
    ...
    override fun onBindViewHolder(holder: FoodHolder, position: Int) {
        ...
        holder.binding.foodimagedetail.getImage("$BASE_URL//FinalProject/foodimages/${list.get(position).food_image}")
    }
}